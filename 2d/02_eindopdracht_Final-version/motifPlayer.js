...
function newGuitarMotif(done) {
    ...
    if (isLeft) {
        guitarAudio = new Audio(guitarLeftMotifArray[Math.floor(Math.random() * guitarLeftMotifArray.length)]);
    } else {
        guitarAudio = new Audio(guitarRightMotifArray[Math.floor(Math.random() * guitarRightMotifArray.length)]);
    }

    const sourceNode = audioCtxt.createMediaElementSource(guitarAudio);
    sourceNode.connect(gainNode);

    guitarAudio.volume = guitarVolumeSlider.value;
    guitarAudio.play();

    guitarAudio.addEventListener('ended', done);

    console.log("guitar mm accoustic");
}
...
guitarVolumeSlider.value = 0;
guitarVolumeSlider.addEventListener('input', () => {
    const vol = guitarVolumeSlider.value;

    if (guitarRightAudio) guitarRightAudio.volume = vol;
    if (guitarLeftAudio) guitarLeftAudio.volume = vol;
    if (electricGuitarAudio2) electricGuitarAudio2.volume = vol;
    if (electricGuitarAudio) electricGuitarAudio.volume = vol;
    if (fluteImrpoAudio) fluteImrpoAudio.volume = vol;
    if (drumAudio) drumAudio.volume = vol;
