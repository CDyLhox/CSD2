...
class OnePole : public Filter {
    ...
    float process(float input) override {
        // Y[n] = bX[n] + aY[n-1]
        // You make this one:
		float output = (b * input) + (a*feedback);
		feedback = input;
		return output;
    }
...
class SimpleLadder :  public Filter {
    public:
						OnePole myFirstOnePole;
    float process(float input) override {
						A = myFirstOnePole.process(input);
						B = myFirstOnePole.process(A);
						C = myFirstOnePole.process(B);
						D = myFirstOnePole.process(C);
						return D; // return D
											//            ,--,
											//      _ ___/ /\|
											//    '( )__, )  ~
